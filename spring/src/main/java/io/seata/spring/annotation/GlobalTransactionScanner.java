    ...
    protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
        try {
            synchronized (PROXYED_SET) {
                ...
                interceptor = null;
                //check TCC proxy: 对于 一般的 TCC 而言, 就是存在标注 @LocalTcc 父接口
                if (TCCBeanParserUtils.isTccAutoProxy(bean, beanName, applicationContext)) {
                    ...
                    // 一般情况实际就是 bean 对应的 Class 类型
                    Class<?> serviceInterface = SpringProxyUtils.findTargetClass(bean);
                    Class<?>[] interfacesIfJdk = SpringProxyUtils.findInterfaces(bean); // 如果不是 Spring-JDK 的代理对象, 则为空

                    // 类/类方法 不存在 GlobalTransactional/GlobalLock 注解, 无需代理, 直接返回 原始 bean
                    if (!existsAnnotation(new Class[]{serviceInterface})
                        && !existsAnnotation(interfacesIfJdk)) {
                        return bean;
                    }

                    // 如果上面没有直接返回 原始 bean, 则此处需要初始化 interceptor. 与 TCC 的 interceptor 不同
                    ...
                    // 这里的 interceptor 在 super.wrapIfNecessary(bean, beanName, cacheKey); 中会使用.
                    // io.seata.spring.annotation.GlobalTransactionScanner.getAdvicesAndAdvisorsForBean
